...
class TaskController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $overdueTasksCount = $allTasks->where('status', 'pending')->where('due_date', '<', now())->count();

        $tasks = $query->orderBy('due_date')->paginate(10)->withQueryString();
        $courses = Course::where('user_id', auth()->id())->get();
            
        return view('tasks.index', compact('tasks', 'courses', 'totalTasks', 'pendingTasksCount', 'completedTasksCount', 'overdueTasksCount'));
    }
