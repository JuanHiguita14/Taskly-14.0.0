...
class TeacherDashboardController extends Controller
{
    ...
    public function grades(Request $request)
    {
        ...
        $gradedTasksQuery = Task::with(['user', 'course'])
            ->whereIn('user_id', $studentIds)
            ->whereNotNull('grade')
            ->where('graded_by', auth()->id());

        $gradedProjectsQuery = Project::with(['user', 'course'])
            ->whereIn('user_id', $studentIds)
            ->whereNotNull('grade')
            ->where('graded_by', auth()->id());
        ...
        if ($request->filled('q')) {
            $q = trim($request->q);
            $gradedTasksQuery->where(function($sub) use ($q) {
                $sub->where('title', 'like', "%{$q}%")
                    ->orWhere('description', 'like', "%{$q}%")
            ...
            $gradedProjectsQuery->where(function($sub) use ($q) {
                $sub->where('name', 'like', "%{$q}%")
                    ->orWhere('description', 'like', "%{$q}%")
                    ->orWhereHas('user', function($u) use ($q) {
                        $u->where('name', 'like', "%{$q}%")
        ...
        $gradedTasks = $gradedTasksQuery->orderBy('graded_at', 'desc')->paginate(15)->withQueryString();
        $gradedProjects = $gradedProjectsQuery->orderBy('graded_at', 'desc')->paginate(15)->withQueryString();
            ...
            foreach ($request->subtasks as $subtaskData) {
                if (!empty($subtaskData['title'])) {
                    \App\Models\ProjectTask::create([
                        ...
                        'due_date' => !empty($subtaskData['due_date']) ? $subtaskData['due_date'] : null,
                        'priority' => $subtaskData['priority'] ?? 2,
                        'order' => $order++,
                        'status' => 'pending'
                    ]);
                }
