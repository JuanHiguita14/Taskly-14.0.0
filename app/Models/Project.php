...
class Project extends Model
{
    ...
    /**
     * Calcula el promedio de las subtareas calificadas.
     */
    public function calculateAverageGrade(): ?float
    {
        $gradedTasks = $this->projectTasks()->whereNotNull('grade')->get();
        
        if ($gradedTasks->isEmpty()) {
            return null;
        }
        
        $totalGrade = $gradedTasks->sum('grade');
        $averageGrade = $totalGrade / $gradedTasks->count();
        
        return round($averageGrade, 2);
    }

    /**
     * Actualiza la calificación del proyecto basada en el promedio de las subtareas.
     */
    public function updateGradeFromSubtasks(): void
    {
