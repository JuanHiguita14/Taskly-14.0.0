...
<div class="max-w-4xl mx-auto">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <dl class="space-y-3">
                        ...
                        @php
                            $averageGrade = $project->calculateAverageGrade();
                        @endphp
                        @if($averageGrade !== null)
                        <div>
                            <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Calificación Final</dt>
                            <dd class="mt-1">
                                <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-indigo-100 dark:bg-indigo-900 text-indigo-800 dark:text-indigo-200">
                                    ...
                                    {{ number_format($averageGrade, 1) }}/100
                                </span>
                                <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">
                                    Promedio de {{ $project->projectTasks()->whereNotNull('grade')->count() }} subtarea(s) calificada(s)
                                </p>
                            </dd>
                        </div>
                        @endif
    ...
    <!-- Calificación Final (promedio de subtareas) -->
    @php
        $averageGrade = $project->calculateAverageGrade();
        $gradedSubtasks = $project->projectTasks()->whereNotNull('grade')->get();
    @endphp
    @if($averageGrade !== null)
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow mb-6">
        <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
            <h3 class="text-lg font-medium text-gray-900 dark:text-white">Calificación Final del Proyecto</h3>
        </div>
        <div class="px-6 py-4">
            <div class="bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-200 dark:border-indigo-800 rounded-lg p-4">
                <div class="flex items-center justify-between">
                    <div>
                        <h4 class="text-sm font-medium text-indigo-800 dark:text-indigo-200">Promedio de Subtareas</h4>
                        <p class="text-3xl font-bold text-indigo-600 dark:text-indigo-300 mt-2">{{ number_format($averageGrade, 1) }}/100</p>
                        <p class="text-sm text-indigo-600 dark:text-indigo-300 mt-2">
                            Basado en {{ $gradedSubtasks->count() }} de {{ $project->projectTasks()->count() }} subtarea(s) calificada(s)
                        </p>
                    </div>
            ...
            @if($gradedSubtasks->count() > 0)
            <div class="mt-4">
                <h5 class="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Desglose de Calificaciones:</h5>
                <div class="space-y-2">
                    @foreach($gradedSubtasks as $subtask)
                    <div class="flex items-center justify-between p-2 bg-gray-50 dark:bg-gray-700 rounded">
                        <span class="text-sm text-gray-700 dark:text-gray-300">{{ $subtask->title }}</span>
                        <span class="text-sm font-medium text-indigo-600 dark:text-indigo-400">{{ number_format($subtask->grade, 1) }}/100</span>
                    </div>
                    @endforeach
                </div>
            </div>
                    ...
                    <div class="w-full md:w-80">
                            ...
                            <div class="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg p-3">
                                <div class="flex items-center justify-between">
                                    <div>
                                        ...
                                        <p class="text-xl font-bold text-green-600 dark:text-green-300">{{ number_format($t->grade, 1) }}/100</p>
                                        @if($t->feedback)
                                        ...
                                        @if($t->graded_at)
                                            <p class="text-xs text-green-500 dark:text-green-400 mt-1">
                                                Calificada el {{ $t->graded_at->format('d/m/Y H:i') }}
                                            </p>
                                        @endif
                                    </div>
                                </div>
                        ...
                        @else
                            <form onsubmit="return gradeSubtask(event, {{ $project->id }}, {{ $t->id }})" class="bg-gray-50 dark:bg-gray-700 rounded-lg p-3 border border-gray-200 dark:border-gray-600">
                                ...
                                <input type="number" name="grade" min="0" max="100" step="0.1" required class="w-full mb-2 rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-white focus:border-indigo-300 focus:ring focus:ring-indigo-200">
    ...
    <div class="flex justify-between items-center">
        ...
        <div class="flex space-x-3">
            ...
            @can('update', $project)
            @if(!(auth()->user()->role === 'student' && $project->teacher_id !== null))
...
function gradeSubtask(event, projectId, subtaskId) {
    ...
    .then(d => { 
        if (d.success) { 
            ...
            alert('Subtarea calificada exitosamente. La calificación del proyecto se ha actualizado automáticamente.');
            location.reload(); 
        } else { 
