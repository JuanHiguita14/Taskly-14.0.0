...
<div class="max-w-6xl mx-auto">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow">
        <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center">
            ...
            @if(auth()->user()->hasAnyRole(['teacher', 'admin']) || ($project->teacher_id !== null && auth()->user()->id === $project->teacher_id))
        ...
        <div id="tasksList" class="divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        <div class="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg p-3 mb-3">
                            <div class="flex items-center justify-between">
                                <div>
                                    ...
                                    <p class="text-xl font-bold text-green-600 dark:text-green-300">{{ number_format($task->grade, 1) }}/100</p>
                                    @if($task->feedback)
                                    ...
                                    @if($task->graded_at)
                                        <p class="text-xs text-green-500 dark:text-green-400 mt-1">
                                            Calificada el {{ $task->graded_at->format('d/m/Y H:i') }}
                                        </p>
                                    @endif
                                </div>
                            </div>
                        </div>
                        @elseif(auth()->user()->hasAnyRole(['teacher', 'admin']) || ($project->teacher_id !== null && auth()->user()->id === $project->teacher_id))
                        <!-- Formulario de calificación para profesores -->
                        <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-3 border border-gray-200 dark:border-gray-600 mb-3">
                            <form onsubmit="return gradeSubtask(event, {{ $project->id }}, {{ $task->id }})">
                                @csrf
                                <label class="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">Calificación (0-100)</label>
                                <input type="number" name="grade" min="0" max="100" step="0.1" required 
                                       class="w-full mb-2 rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-white focus:border-indigo-300 focus:ring focus:ring-indigo-200">
                                <label class="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">Comentarios (opcional)</label>
                                <textarea name="feedback" rows="2" 
                                          class="w-full mb-2 rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-white focus:border-indigo-300 focus:ring focus:ring-indigo-200"
                                          placeholder="Comentarios sobre la subtarea..."></textarea>
            ...
            <div class="p-12 text-center">
                ...
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                    ...
                    @else
                        El profesor aún no ha agregado subtareas a este proyecto.
                    @endif
                </p>
                @if(auth()->user()->hasAnyRole(['teacher', 'admin']) || ($project->teacher_id !== null && auth()->user()->id === $project->teacher_id))
                <div class="mt-6">
