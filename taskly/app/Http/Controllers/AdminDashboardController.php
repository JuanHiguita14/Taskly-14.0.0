...
class AdminDashboardController extends Controller
{
    ...
    public function storeAssignment(Request $request)
    {
        ...
        if (!$course) {
            $course = \App\Models\Course::create([
                'user_id' => auth()->id(), // El admin actual crea el curso
                'name' => $courseName,
                'code' => $courseCode,
    ...
    public function downloadUsersReport()
    {
        ...
        $filename = 'reporte_usuarios_' . date('Y-m-d') . '.csv';
     ...
     */
    public function downloadTeachersPerformanceReport()
    {
        $teachers = User::where('role', 'teacher')
            ->with(['students', 'students.tasks', 'students.projects'])
            ->get();
        
        $filename = 'reporte_rendimiento_profesores_' . date('Y-m-d') . '.csv';
        ...
        $callback = function() use ($teachers) {
            ...
            foreach ($teachers as $teacher) {
                ...
                $totalTasks = Task::whereIn('user_id', $studentIds)->count();
                $totalProjects = Project::whereIn('user_id', $studentIds)->count();
                $gradedTasks = Task::whereIn('user_id', $studentIds)->whereNotNull('grade')->count();
                $gradedProjects = Project::whereIn('user_id', $studentIds)->whereNotNull('grade')->count();
                ...
                if ($students->count() > 0) {
                    ...
                    foreach ($students as $student) {
                        $studentTaskGrades = $student->tasks()->whereNotNull('grade')->pluck('grade');
                        $studentProjectGrades = $student->projects()->whereNotNull('grade')->pluck('grade');
                        ...
                        fputcsv($file, [
                            ...
                            $student->tasks()->whereNotNull('grade')->count(),
                            $student->projects()->whereNotNull('grade')->count(),
