...
<div class="max-w-4xl mx-auto">
    ...
    @if(auth()->user()->hasAnyRole(['teacher', 'admin']) && $project->isSubmitted())
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow mb-6">
        <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
        ...
        <div class="px-6 py-4">
            @if($project->grade !== null)
                <div id="gradeDisplay{{ $project->id }}" class="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg p-4">
                    <div class="flex items-center justify-between">
                        <div>
                            <h4 class="text-sm font-medium text-green-800 dark:text-green-200">Calificación Asignada</h4>
                            <p class="text-2xl font-bold text-green-600 dark:text-green-300">{{ number_format($project->grade, 0) }}/100</p>
                            @if($project->feedback)
                                <p class="text-sm text-green-600 dark:text-green-300 mt-1">{{ $project->feedback }}</p>
                            @endif
                            <p class="text-xs text-green-500 dark:text-green-400 mt-1">
                                Calificado el {{ $project->graded_at->format('d/m/Y H:i') }}
                                @if($project->grader)
                                    por {{ $project->grader->name }} {{ $project->grader->last_name }}
                                @endif
                            </p>
                        </div>
                        <button onclick="editGrade({{ $project->id }}, {{ $project->grade }}, '{{ addslashes($project->feedback ?? '') }}')" 
                                class="text-green-600 hover:text-green-800 dark:text-green-400 dark:hover:text-green-200">
                            <i class="fas fa-edit"></i>
                        </button>
                    </div>
                </div>
                <div id="gradeEditForm{{ $project->id }}" class="hidden">
                    <form onsubmit="updateGrade(event, {{ $project->id }})">
                        @csrf
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Calificación (0-100)</label>
                                <input type="number" 
                                       id="editGrade{{ $project->id }}"
                                       name="grade" 
                                       min="0" 
                                       max="100" 
                                       step="1"
                            ...
                            <div>
                                ...
                                <textarea name="feedback" 
                                          id="editFeedback{{ $project->id }}"
                                          rows="3"
                                          class="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200"
                                          placeholder="Comentarios sobre el proyecto...">{{ $project->feedback }}</textarea>
                            </div>
                        </div>
                        <div class="mt-4 flex space-x-2">
                            ...
                            <button type="button" 
                                    onclick="cancelEditGrade({{ $project->id }})"
            ...
            @else
                <form id="gradeForm" onsubmit="submitGrade(event, {{ $project->id }})">
                    @csrf
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Calificación (0-100)</label>
                        ...
                        <div>
                            ...
                            <textarea name="feedback" 
                                      rows="3"
                                      class="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200"
                                      placeholder="Comentarios sobre el proyecto..."></textarea>
                        </div>
                    </div>
                    <div class="mt-4">
                        <button type="submit" 
                                class="bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-md text-sm font-medium">
                            <i class="fas fa-check mr-2"></i> Calificar Proyecto
                        </button>
                    </div>
                </form>
            @endif
...
function submitGrade(event, projectId) {
    ...
    fetch(`/projects/${projectId}/grade`, {
        method: 'POST',
        body: formData,
        headers: {
            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
...
function editGrade(projectId, currentGrade, currentFeedback) {
    // Ocultar la vista de calificación
    document.getElementById('gradeDisplay' + projectId).classList.add('hidden');
    // Mostrar el formulario de edición
    document.getElementById('gradeEditForm' + projectId).classList.remove('hidden');
    
    // Establecer valores actuales
    document.getElementById('editGrade' + projectId).value = currentGrade;
    document.getElementById('editFeedback' + projectId).value = currentFeedback || '';
}

function cancelEditGrade(projectId) {
    // Ocultar el formulario de edición
    document.getElementById('gradeEditForm' + projectId).classList.add('hidden');
    // Mostrar la vista de calificación
    document.getElementById('gradeDisplay' + projectId).classList.remove('hidden');
}

function updateGrade(event, projectId) {
    event.preventDefault();
